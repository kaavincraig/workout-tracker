...
| 6 | **Session stats** — est 1RM, total volume, sets done | ⬜ | "Session summary" screen (Strong/Hevy). Computed from existing logs. |
...
### Wave 2 — stats layer (recommended next)
- #6 Session stats *(cheapest, highest value)*
...
- **⏱ Rest Timer** — hidden by default; enable in ⚙️ Settings. Global floating bar with presets (30s / 60s / 90s / 2m / 3m), haptic + chime on finish. State persists in `localStorage` (`restTimerEnabled`).

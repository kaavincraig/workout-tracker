...
## Version history
- **1.8.0 (uncommitted, in-progress)** — Settings → 🎨 Theme picker (4: Midnight Slate default + Steel Cobalt + Graphite Mono + Aurora Teal). Earlier commits 1f7f531 + af4568b shipped 6 (added Glacier + Ivory Chalk, both light); the two light themes have since been removed (their `[data-theme]` CSS blocks, the scoped light-theme text override, and the `THEMES` entries are gone). Themes = `[data-theme="…"]` CSS blocks + `THEMES` array; persists `appTheme`, applied on load (default removes `data-theme` attr; unknown id → default). Accent tints via `--accent-rgb`, accent text via `--on-accent`.

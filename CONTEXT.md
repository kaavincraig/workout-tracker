...
> **v1.8.0 (uncommitted)** — adds a **🎨 Theme picker** to the ⚙️ Settings tab. Options so far: **Midnight Slate** (default), **Steel Cobalt** (`#5b8def`), **Graphite Mono** (neutrals), **Aurora Teal** (`#2dd4bf`), **Glacier** (light frost + royal-blue `#2563eb`), **Ivory Chalk** (light bone + forest-green `#1f6f43`). Adding a theme = a `[data-theme="…"]` CSS block (sets the same `--*` vars as `:root`, incl. `--accent-rgb` + `--on-accent`) + one entry in the `THEMES` array (index.html). Selection persists to `localStorage` as `appTheme` and is applied on load (removes the `data-theme` attr for default, sets `data-theme="<id>"` otherwise). Accent hovers/tints use `rgba(var(--accent-rgb), …)` and accent-button text uses `var(--on-accent)` (instead of hardcoded near-black) so any future accent follows its theme. **Light themes (glacier/ivory-chalk)** get a scoped override block (right after the theme var blocks) switching themed-surface text (header/day-title/accordion/stepper/exercise-name/timer-digits/inputs/labels +   `.exhist-chip:not(.pr)` → accent) to `var(--text-main)`; always-dark surfaces (rest-timer bar, `#0b1120` value wells, dark-red/gold chips, `rgba(15,23,42,…)` boxes) stay white. **Follow-up (1f7f531+):** `.exercise-name` (and the dimmed state `.exercise-main-row.is-dimmed .exercise-name`) explicitly forced in the light-theme block to `var(--text-main)` / `#475569` with `!important`, because the base `.exercise-name{color:#fff}` and the dimmed rule (`#64748b !important`) were winning the cascade → low-contrast names in both light themes. Inline `style="color:#fff"` on themed surfaces was converted to `var(--text-main)`.

> **v1.7.0 (committed, 83e2a03)** — implements PLAN #1 **Progress Charts**. New 📈 tab (between + and 📊) shows a per-exercise line chart over time, drawn as pure inline `<svg>` (no deps). Metric toggle: **Peak set** (heaviest loaded set per session) or **Volume (lbs)** (Σ weight × reps). Each card: exercise name, # of sessions, best value, ★ on the peak point, per-dot tooltip (date + value). Only exercises with a loaded set in **2+ sessions** are charted (bodyweight-only / single-session excluded). Engine: `collectExerciseNames` / `getExerciseProgress` / `buildLineSVG` / `renderProgressTab` / `switchProgressMetric` (after `renderStatsTab`). Read-only, computed from `workout_master_logs`, no new data/deps. Tab wired into `renderWorkoutForms` (button), `.tab-content` div `#progressTabContent`, `switchTab` allow-list + render dispatch. Headless test: `progress_test.mjs` in the opencode scratch dir.

> **v1.6.0 (committed, ddbd221)** — implements PLAN #6 **Session Stats** + trims the rest-timer presets. New 📊 tab (between + and 🏆) shows: all-time summary (workouts, total sets, total/avg volume in lbs) + per-session rows (newest first) with sets done, total volume, and best estimated 1RM of any loaded set (Epley `weight × (1 + reps/30)`), naming the exercise + set that produced it. Bodyweight / zero-rep rows excluded. Engine: `estOneRM` / `computeSessionStats(log)` / `renderStatsTab` (right after `renderPRsTab`). Read-only, computed from `workout_master_logs`, no new data/deps. Tab wired into `renderWorkoutForms` (button), the `.tab-content` div `#statsTabContent`, `switchTab` allow-list + render dispatch. Rest-timer presets trimmed to **30s / 60s / 90s** (2m/3m removed). App version → 1.6.0. Headless test: `stats_test.mjs` in the opencode scratch dir.
...
## Version history
- **1.8.0 (uncommitted)** — Settings → 🎨 Theme picker (6: Midnight Slate default + Steel Cobalt + Graphite Mono + Aurora Teal + Glacier + Ivory Chalk). Themes = `[data-theme="…"]` CSS blocks + `THEMES` array; persists `appTheme`, applied on load. Accent tints via `--accent-rgb`, accent text via `--on-accent`. Light themes get a scoped text-readability override (themed surfaces → `var(--text-main)`, always-dark surfaces stay white).
- **1.7.0 (committed, 83e2a03)** — #1 Progress Charts: 📈 tab with per-exercise line charts (peak set / volume), pure SVG, only 2+ session exercises charted. `collectExerciseNames` / `getExerciseProgress` / `buildLineSVG` / `renderProgressTab` / `switchProgressMetric`.
- **1.6.0 (committed, ddbd221)** — #6 Session Stats: 📊 tab with all-time summary (workouts / total sets / total + avg volume) + per-session sets, volume, best est 1RM (Epley). Bodyweight excluded. `estOneRM` / `computeSessionStats` / `renderStatsTab`. Rest-timer presets trimmed to 30s/60s/90s.
...
- **v1.7.0** — `collectExerciseNames` / `getExerciseProgress` / `buildLineSVG` / `renderProgressTab` / `switchProgressMetric` — right after `renderStatsTab`.
- **v1.8.0** — `THEMES` / `getStoredTheme` / `isKnownTheme` / `applyTheme` / `renderThemePicker` — right after `toggleRestTimerEnabled`.

...
> **v1.8.0 (committed, d479591)** — adds a **🎨 Theme picker** to the ⚙️ Settings tab. Options: **Midnight Slate** (default), **Steel Cobalt** (`#5b8def`), **Graphite Mono** (neutrals), **Aurora Teal** (`#2dd4bf`). (Earlier commits 1f7f531 + af4568b shipped a 6th pair — **Glacier** (light) + **Ivory Chalk** (light) — which d479591 removed.) Adding a theme = a `[data-theme="…"]` CSS block (sets the same `--*` vars as `:root`, incl. `--accent-rgb` + `--on-accent`) + one entry in the `THEMES` array (index.html). Selection persists to `localStorage` as `appTheme` and is applied on load (removes the `data-theme` attr for `default`, sets `data-theme="<id>"` otherwise; **unknown/removed ids fall back to default**). Accent hovers/tints use `rgba(var(--accent-rgb), …)` and accent-button text uses `var(--on-accent)` so any future accent follows its theme. When adding a LIGHT theme in the future: it will need a scoped CSS block (after the theme var blocks, before `* {`) that switches themed-surface text to `var(--text-main)` and `.exhist-chip:not(.pr)` to `var(--accent)`, forces `.exercise-name` (normal + `.is-dimmed`) with `!important` to beat the base/dimmed rules, and the inline `style="color:#fff"` on themed surfaces to be `var(--text-main)` (or overridden in the light-theme block). See af4568b for an example of the light-theme override structure.
...
## Version history
- **1.8.0 (d479591, building on 1f7f531 + af4568b)** — Settings → 🎨 Theme picker with 4 dark themes (Midnight Slate default + Steel Cobalt + Graphite Mono + Aurora Teal). 1f7f531 + af4568b originally shipped 6 (added Glacier + Ivory Chalk, both light); d479591 removed the two light themes (their `[data-theme]` CSS blocks, the scoped light-theme text override, and the `THEMES` entries). Themes = `[data-theme="…"]` CSS blocks + `THEMES` array; persists `appTheme`, applied on load (default removes `data-theme` attr; unknown id → default). Accent tints via `--accent-rgb`, accent text via `--on-accent`.

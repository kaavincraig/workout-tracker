...
## Version history
- **1.7.0 (uncommitted)** — #1 Progress Charts: 📈 tab with per-exercise line charts (peak set / volume), pure SVG, only 2+ session exercises charted. `collectExerciseNames` / `getExerciseProgress` / `buildLineSVG` / `renderProgressTab` / `switchProgressMetric`.
